...
pipeline {
    ...
    stages {
        ...
        stage("Exporting Apps from Apigee") {
            steps {
            script {
            def currentDate = new Date().format("dd-MM-yyyy")
                    def exportDir = "<APIGEE_ORG_NAME>/${currentDate}/apps"
                    
                    sh """
                    mkdir -p ${exportDir}
                    ${APIGEE_CLI_DIR}/apigeecli apps export -o <APIGEE_ORG_NAME> -t ${TOKEN} 
                    """
                    sh "mv apps.json ${exportDir}"
            ...
            if (changes) {
                sh "git commit -m 'Added apps from Jenkins Pipeline'"
                withCredentials([gitUsernamePassword(credentialsId: "${GITHUB_CREDS}", gitToolName: 'Default')]) {
                    sh "git push -u origin main"
                }
            } else {
                echo "No changes to commit in apps. Skipping git push."
                    }
                }
            }
        }
